    ...
    public class FileCache : IDisposable
    {
            ...
            using (new ReadLock(_lockEx)) {
                foreach (var entry in _entriesByKey.Keys) {
                    try {
                        ...
                        // directory
                        if (slashPos > -1) {
                            files.Add(entry[start..(slashPos + 1)]);
                        }
                        // file
                        else {
                            files.Add(entry[start..]);
                        }
                    }
                    catch { }
                }
        ...
        private class FileCacheEntry
        {
            ...
            private static void OnCreated(object sender, FileSystemEventArgs e, FileCache cache, FileCacheEntry entry)
            {
                ...
                cache.InsertFileInternal(entry._path, file, key, entry._timespan, entry._handler, maxTry: 3);
            }
            ...
            private static void OnRenamed(object sender, RenamedEventArgs e, FileCache cache, FileCacheEntry entry)
            {
                ...
                cache.InsertFileInternal(entry._path, newFile, newKey, entry._timespan, entry._handler, maxTry: 3);
            }
        };

        private bool InsertFileInternal(string path, string file, string key, TimeSpan timeout, InsertHandler handler, int maxTry = 1)
        {
            key = key.Replace('\\', '/').Replace("//", "/");
            file = file.Replace('\\', '/').Replace("//", "/");

            // Depending the tool (ie: vscode, sublime text...) used
            // to edit a file while server is running, events like OnChanged()
            // can be fire multiple time with read or write lock by the
            // underlying saving process.
            // So ReadAllBytes() will at best thrown an exception and
            // sometimes return 0 byte content.
            // Workaround : insert a 250ms sleep between each 2 first attempts
            //              the first return as regular
            // use : to avoid long wait on first launch, we only use maxtry
            //       on events (created, changed, renamed)
            for (var i = 0; i < maxTry; i++)
            {
                try
                {
                    ...
                    // Empty file considered as read error
                    if (content.Length != 0 || i == maxTry - 1)
                    {
